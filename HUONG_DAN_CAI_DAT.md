...
| `BYTEPLUS_API_KEY` | Khuyến nghị | Bật chat AI/insight thật qua LLM. Không có vẫn chạy được (rule-based) |
| `USE_GMAIL_API=true` + `gmail_credentials.json`/`gmail_token.json` | Tuỳ chọn | Bật đối soát email thật (nhận file riêng từ đội, không commit git). Thiếu thì 3 endpoint đối soát email báo lỗi, phần còn lại chạy bình thường |
