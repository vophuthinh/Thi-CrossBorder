...
def dashboard_reporting_send_email(req: ReportEmailRequest):
    ...
    # Spec requires confirmation before any self-notify send — this
    # endpoint used to send on the very first call, with no draft-review
    # step (found while merging this feature in). Callers must now see the
    # draft (status="draft") and re-POST with confirmed=true to actually send.
